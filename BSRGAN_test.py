...
model = RRDBNet(sf = args.sr_ratio).eval().to(args.device)
if args.sr_ratio == 4:
    model.load_state_dict(torch.load('.\\BSRGAN.pth'))
else:
    model.load_state_dict(torch.load('.\\BSRGANx2.pth'))    
image = torch.from_numpy(cv2.cvtColor(cv2.imread(args.image_path),cv2.COLOR_BGR2RGB)/255)[None].permute(0,3,1,2).to(args.device).float()
...
with torch.no_grad():
    image = model(image)
cv2.imwrite(save_path,cv2.cvtColor(torch.clip(image[0].permute(1,2,0)*255,0,255).to(torch.uint8).cpu().numpy(),cv2.COLOR_RGB2BGR))
